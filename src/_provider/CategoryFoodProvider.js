...
export const CategoryFoodProvider = ({ children }) => {
  ...
  const createCategory = async (payload) => {
    const { data } = await axios.post("http://localhost:1000/category", payload);
    setCategories(prev => [data, ...prev]);
    return data;
  };

  const updateCategory = async (id, payload) => {
    const { data } = await axios.put(`http://localhost:1000/category/${id}`, payload);
    setCategories(prev => prev.map(c => c._id === id ? data : c));
    return data;
  };
  ...
  const fetchFoodByCategory = (categoryId) => {
    // categoryId-ээр filter хийж буцаах
  ...
  const updateFood = async (id, payload) => {
    const { data } = await axios.put(`http://localhost:1000/food/${id}`, payload);
    setFoods(prev => prev.map(f => f._id === id ? data : f));
    triggerRefresh();
    return data;
  };
  ...
  return (
    <AppContext.Provider value={{
      categories, catLoading, fetchCategories, createCategory, updateCategory, deleteCategory,
      foods, foodLoading, fetchFood, createFood, updateFood, deleteFood, fetchFoodByCategory,

...
export default function FoodDialog({ open, setOpen, food, addToCart }) {
      ...
      <DialogContent className="p-0 rounded-2xl w-[760px] max-w-none">
        <div className="grid grid-cols-2 gap-4 p-4">
          ...
          <div className="pr-4 relative">
            ...
            <h2 className="text-red-500 text-xl font-semibold">{food.foodName}</h2>
            <p className="text-gray-600 mt-1 text-sm">{food.ingredients}</p>

            
            <div className="mt-6">
              <p className="text-sm text-gray-500">Total price</p>
              <p className="text-lg font-bold">${(food.price * qty).toFixed(2)}</p>
            </div>
